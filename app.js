...
class TelemarketingDashboard {
        ...
        this.data.mesi.forEach(mese => {
            ...
            monthCard.innerHTML = `
                ...
                <div class="results-grid">
                    ...
                    <div class="result-item">
                        <div class="result-label">Valore Cond. Potenziali</div>
                        <div class="result-value" id="${mese}_valoreCondominii">€0</div>
                    </div>
                    <div class="result-item">
    ...
    setupEventListeners() {
        ...
        this.data.mesi.forEach(mese => {
            const inputs = [
                ...
                'appuntamentiPercorsi', 'condominiiPotenziali', 'potenzialePDR', 'potenzialePOD'
            ];
    ...
    exportToCSV() {
        try {
            let csvContent = 'Mese,Chiamate Partite,Chiamate Effettive,Non Risponde,Non Interessato,Numero Errato,Appuntamenti Fissati,Appuntamenti Percorsi,Ore Lavorate,Costo Orario,Costo Totale,Condominii Potenziali,POD Potenziali,PDR Potenziali,Valore POD,Valore PDR,Totale Potenziale,ROI %\n';
    ...
    updateMonthData(mese) {
        ...
        data.condominiiPotenziali = parseInt(document.getElementById(`${mese}_condominiiPotenziali`).value) || 0;
    ...
    updateMonthResults(mese) {
        ...
        const chiamateEffettive = data.nonInteressato + data.appuntamentiFissati;
        const costoTotale = data.oreLavorate * data.costoOrario;
        const valoreCondominii = data.condominiiPotenziali * this.data.valoreCondominio;
        const valorePOD = data.potenzialePOD * this.data.valorePOD;
        const valorePDR = data.potenzialePDR * this.data.valorePDR;
        const totalePotenziale = valorePOD + valorePDR;
        ...
        document.getElementById(`${mese}_valoreCondominii`).textContent = `€${valoreCondominii.toLocaleString()}`;
    ...
    initFunnelChart() {
        ...
        this.charts.funnel = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: ['Chiamate Partite', 'Chiamate Effettive', 'App. Fissati', 'App. Percorsi', 'Totale Acquisito Potenziale'],
                datasets: [{
                    ...
                    x: {
                        ...
                        ticks: {
                            callback: function(value, index) {
                                ...
                                if (labels[index] === 'Totale Acquisito Potenziale') {
                                    return '€' + value.toLocaleString();
                                }
                                return value;
                            }
    ...
    updateChiamateChart() {
        const data = this.data.mesi.map(mese => {
            const monthData = this.monthlyData[mese];
            return monthData.nonInteressato + monthData.appuntamentiFissati;
        });

        this.charts.chiamate.data.datasets[0].data = data;
    ...
    updateFunnelChart() {
        ...
        let chiamatePartite = 0, chiamateEffettive = 0, appFissati = 0, appPercorsi = 0, totaleAcquisitoPotenziale = 0;
        ...
        if (selectedMonth === 'all') {
            ...
            this.data.mesi.forEach(mese => {
                ...
                totaleAcquisitoPotenziale += (data.potenzialePOD * this.data.valorePOD) + (data.potenzialePDR * this.data.valorePDR);
            });
        } else {
        ...
        this.charts.funnel.data.datasets[0].data = [
            chiamatePartite, chiamateEffettive, appFissati, appPercorsi, totaleAcquisitoPotenziale
        ];
        this.charts.funnel.update();

        // Update funnel stats
        this.updateFunnelStats(chiamatePartite, chiamateEffettive, appFissati, appPercorsi, totaleAcquisitoPotenziale);
    }

    updateFunnelStats(chiamatePartite, chiamateEffettive, appFissati, appPercorsi, totaleAcquisito) {
        ...
        funnelMetrics.innerHTML = `
            <div class="funnel-metric">
                <span class="metric-label">Tasso Efficacia</span>
            ...
            <div class="funnel-metric">
                <span class="metric-label">Valore Medio per Chiamata</span>
                <span class="metric-value">€${chiamatePartite > 0 ? (totaleAcquisito / chiamatePartite).toFixed(0) : 0}</span>
            </div>
            <div class="funnel-metric">
                <span class="metric-label">Valore Medio per App.</span>
                <span class="metric-value">€${appPercorsi > 0 ? (totaleAcquisito / appPercorsi).toFixed(0) : 0}</span>
            </div>
        `;
    }
    ...
    updateSummaryTable() {
        ...
        this.data.mesi.forEach(mese => {
            ...
            row.innerHTML = `
                ...
                <td>${data.condominiiPotenziali}</td>
